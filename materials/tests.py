...
class LessonsCreateTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.data = {"title": "beginning", "description": "course on python programming",
                     "video_link": "https://youtube.com", "course": self.course.id}
    ...
    def test_lesson_create(self):
        ...
        self.assertEqual(response.json(),
                         {'id': 1, 'title': 'beginning', 'description': 'course on python programming',
                          'preview': None, 'video_link': 'https://youtube.com', 'course': 1, 'owner': 1})

    def test_lesson_create_invalid(self):
        self.data = {"title": "beginning", "description": "course on python programming",
                     "video_link": "https://lajflajl.com", "course": self.course.id}

        response = self.client.post("/lessons/create/", self.data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json(), {
            'non_field_errors': ['Нельзя использовать ссылки на сторонние образовательные платформы или личные сайты']})
...
class LessonsTestCase(APITestCase):
    def setUp(self):
        ...
        self.course = Course.objects.create(title="test", description="test")
        self.lesson = Lesson.objects.create(title="beginning", description="test",
                                            video_link="https://youtube.com", course=self.course, owner=self.user_owner)
    ...
    def test_lesson_list_owner(self):
        ...
        self.assertEqual(response.json(), {'count': 1, 'next': None, 'previous': None, 'results': [
            {'id': self.lesson.id, 'title': 'beginning', 'description': 'test', 'preview': None, 'video_link': 'https://youtube.com',
    ...
    def test_lesson_retrieve_owner(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json(), {'id': self.lesson.id, 'title': 'beginning', 'description': 'test', 'preview': None,
    ...
    def test_lesson_retrieve_moderator(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json(), {'id': self.lesson.id, 'title': 'beginning', 'description': 'test', 'preview': None,
                                           'video_link': 'https://youtube.com', 'course': self.course.id, 'owner': self.user_owner.id})
    ...
    def test_lesson_update_owner(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json(), {'id': self.lesson.id, 'title': 'beginning', 'description': 'new desc', 'preview': None,
                                           'video_link': 'https://youtube.com', 'course': self.course.id, 'owner': self.user_owner.id})
        self.assertTrue(Lesson.objects.filter(id=self.lesson.id).exists())

    def test_lesson_update_invalid(self):
        self.client.force_authenticate(user=self.user_owner)
        self.data_invalid = {"video_link": "https://lajflajl.com",}
