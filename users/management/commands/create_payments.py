...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        method2 = "Наличные"
        created_payment1, created1 = Payment.objects.get_or_create(
            user=user, course=course, amount=amount1, method=method1
        )
        created_payment2, created2 = Payment.objects.get_or_create(
            user=user, lesson=lesson, amount=amount2, method=method2
        )
        if created1:
